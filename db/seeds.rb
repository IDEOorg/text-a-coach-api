...
platform1 = Platform.where(name: "Text A Coach").first_or_create
platform2 = Platform.where(name: "Trust").first_or_create
...
platform1.conversations.where(summary_question: "How much should I contribute to my 401k?").first_or_create({
  user_id: user1.id,
  agent_id: agent1.id,
...
platform1.conversations.where(summary_question: "How can I get rid of credit card debt?").first_or_create({
  user_id: user2.id,
  agent_id: agent1.id,
...
platform1.conversations.where(summary_question: "How do I check my credit score?").first_or_create({
  user_id: user2.id,
  agent_id: agent2.id,
...
platform2.conversations.where(summary_question: "How much should I contribute to my 401k?").first_or_create({
  user_id: user2.id,
  agent_id: agent2.id,
  summary_answer: "Saving 10%% of your earnings for retirement is the general goal. If your Employer offers matching, try to contribute at least enough to get the match.",
  last_message_at: Date.today - 5.days,
